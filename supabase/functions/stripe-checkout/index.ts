...
Deno.serve(async (req) => {
  try {
    ...
    if (!customer || !customer.customer_id) {
      ...
      console.log(`Created new Stripe customer ${newCustomer.id}${user ? ` for user ${user.id}` : ' (guest)'}`);
      ...
      console.log(`Successfully set up new customer ${customerId}${mode === 'subscription' ? ' with subscription record' : ''}`);
    } else {
